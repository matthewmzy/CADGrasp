...
from LASDiffusion.utils.utils import get_voxel_coordinates, VIT_PATCH_NUMBER, VIEW_IMAGE_RES
...
class ResnetBlock(nn.Module):
    def __init__(self, world_dims: int, dim_in: int, dim_out: int, emb_dim: int, dropout: float = 0.1,
                 use_text_condition: bool = False, use_time_condition: bool = True):
        super().__init__()
        self.world_dims = world_dims
        ...
        self.res_conv = conv_nd(
            world_dims, dim_in, dim_out, 1)# if dim_in != dim_out else nn.Identity()   # CHANGE 

    def forward(self, x, time_emb, text_condition=None):
        h = self.block1(x)
        if self.use_text_condition:
            h *= self.text_mlp(text_condition)[(...,) + (None, )*self.world_dims]
