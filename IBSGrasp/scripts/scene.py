...
class Scene:
    def __init__(self, cfg):
        ...
        if self.visualize:
            self.vis = Vis(robot_name=self.robot_name,
            ...
            self.coord_plotly = []
        if self.grasp_num == 0:
            cprint(f"Set not to load grasps from downloaded dataset")
        else:
    ...
    def set_configs(self, cfg):
        ...
        self.meta_path = cfg.meta_path
        self.camera = cfg.camera
        self.scene_id = cfg.scene_id
        self.scene_name = 'scene_' + str(self.scene_id).zfill(4)
        self.grasp_num = cfg.grasp_num
        self.output_path = cfg.output_path
        self.with_graspness = cfg.with_graspness
        self.visualize = cfg.visualize
        self.table_size = cfg.table_size
        self.num_samples = cfg.num_samples
        self.load_gt_ibs = cfg.load_gt_ibs
        self.device = cfg.device

    def calculate_mesh_pc(self):
        scene_path = os.path.join('data/scenes', self.scene_name)
    ...
    def get_camera_pose(self, view):
        self.cam0_wrt_table = np.load(os.path.join('data/scenes', self.scene_name, self.camera, 'cam0_wrt_table.npy'))
        self.camera_pose_wrt_cam0 = np.load(os.path.join('data/scenes', self.scene_name, self.camera, 'camera_poses.npy'))[view]
    ...
    def load_ibs(self):
        ibs_data_path = '/DATA/disk0/zhiyuanma/DexGraspNet2/IBSGrasp/ibsdata'
...
@hydra.main(version_base="v1.2", config_path='../conf', config_name='scene')
def main(args):
    set_seed(args.seed)
    scene = Scene(args)
    if args.visualize:
