...
def interpolate_traj(traj, num_steps):
    """
    Interpolates a trajectory using PyTorch's interpolate function.

    Params:
        traj: torch.Tensor of shape [num_particles, lens, n_dofs]
        num_steps: int, desired number of time steps

    Returns:
        new_traj: torch.Tensor of shape [num_particles, num_steps, n_dofs]
    """
    num_particles, lens, n_dofs = traj.shape
    
    # Reshape to [num_particles * n_dofs, 1, lens] for interpolation
    traj_reshaped = traj.permute(0, 2, 1).reshape(num_particles * n_dofs, 1, lens)
    
    # Interpolate along the time dimension (size=lens to size=num_steps)
    interpolated = F.interpolate(traj_reshaped, size=num_steps, mode='linear', align_corners=False)
    
    # Reshape back to [num_particles, num_steps, n_dofs]
    new_traj = interpolated.reshape(num_particles, n_dofs, num_steps).permute(0, 2, 1)
